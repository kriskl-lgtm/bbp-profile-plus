 ...
 *
 * Three-layer protection on WordPress registration (and optionally WP login):
 *   1. Honeypot   - invisible field; bots fill it, humans don't
 *   2. Time gate  - form must be open >= 4 seconds before submit
 *   3. Math CAPTCHA - server-side question, answer stored in a WP transient
 *                    (answer is NOT in the HTML, so bots can't scrape it)
 *
 * Hooks into:
 *   - wp_login_form / register_form  (WordPress default forms)
 *   - register_post / registration_errors (validation)
...
class BBPPP_AntiSpam {
  ...
  private function __construct() {
    ...
    // ---- Enqueue tiny inline JS (sets timestamp on page load) ----
  ...
  public function generate_captcha() {
    ...
    // Use written operator for friendlier display
    $op_labels = array( '+' => '+', '-' => '&#8722;', '*' => '&times;' );
    $question  = $a . ' ' . $op_labels[ $op ] . ' ' . $b;

    // Create a unique token; store hashed answer as transient (10 min)
    ...
    return array(
      'question' => $question,
      'answer'   => $answer,  // only used server-side for debug; not sent to browser
      'token'    => $token,
    );
  }
  ...
  /**
   * Outputs all three anti-spam fields into any form.
   * Called by both the WP registration form and bbPress forms.
   */
  public function render_fields() {
    ...
    <div class="bbppp-antispam-wrap">
      ...
      <?php /* 1. HONEYPOT - hidden with CSS, not type=hidden (bots fill hidden fields too) */ ?>
      ...
      <?php /* 2. TIME GATE - timestamp set by JS on page load */ ?>
      ...
      <p class="bbppp-captcha-row">
        <label for="bbppp_captcha_answer">
          ...
          printf(
            ...
            '<strong class="bbppp-captcha-q">' . $captcha['question'] . '</strong>'
          );
          ?>
  ...
  /**
   * Core validation logic shared between WP registration and bbPress.
  ...
  private function check_submission() {
    ...
    if ( ! empty( $_POST['bbppp_hp_name'] ) ) {
      return new WP_Error(
        'bbppp_spam',
        __( 'Spam check failed.', 'bbp-profile-plus' )
      );
    }
    ...
    // Passed — delete the transient so it can't be reused
  ...
  public function validate_bbpress( $topic_data ) {
    if ( is_user_logged_in() ) return $topic_data; // only check guests/new registrations
    $result = $this->check_submission();
    if ( is_wp_error( $result ) ) {
  ...
  // =========================================================
  // FOOTER JS — sets hidden timestamp field on page load
  // =========================================================

  public function footer_script() {
    // Only run on pages that have our anti-spam fields
    ?>
    <script>
    (function(){
  ...
  // =========================================================
  // ADMIN SETTINGS (optional toggle)
  // =========================================================
