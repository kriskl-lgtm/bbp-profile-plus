...
class BBPPP_Router {
    ...
    public static function register_rewrite_rules() {
        ...
        // Registration route
        add_rewrite_rule( '^register/?$', 'index.php?bbppp_register=1', 'top' );
    }
    ...
    public function resolve() {
        global $wp_query;
        // Registration page
        if ( get_query_var( 'bbppp_register' ) ) {
    ...
    public function template_include( $template ) {
        ...
        if ( 'account' === $this->action ) {
            if ( ! is_user_logged_in() || ( get_current_user_id() !== $this->member->ID && ! current_user_can( 'manage_options' ) ) ) {
                wp_safe_redirect( $this->member_url( $this->member ) );
                exit;
            }
    ...
    public function is_register_page() { return $this->is_register; }
    ...
    function bbppp_render_user_topics( $user_id ) {
        if ( function_exists( 'bbp_get_user_topics_started' ) ) {
            ...
            if ( $q->have_posts() ) {
                ...
                echo '<p class="bbppp-empty">' . esc_html__( 'No topics started yet.', 'bbp-profile-plus' ) . '</p>';
            }
        }
    }
}
if ( ! function_exists( 'bbppp_render_user_replies' ) ) {
    function bbppp_render_user_replies( $user_id ) {
        ...
        if ( $q->have_posts() ) {
            ...
            while ( $q->have_posts() ) { $q->the_post();
                echo '<li><a href="' . esc_url( get_permalink() ) . '">' . esc_html( get_the_title() ) . '</a> &mdash; ' . esc_html( get_the_date() ) . '</li>';
            }
            echo '</ul>';
            wp_reset_postdata();
        } else {
            echo '<p class="bbppp-empty">' . esc_html__( 'No replies yet.', 'bbp-profile-plus' ) . '</p>';
        }
    }
}
