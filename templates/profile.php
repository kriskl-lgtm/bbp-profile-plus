...
// Get member from query var (set by router)
$bbppp_user_obj = get_query_var( 'bbppp_user' );
$bbppp_tab_val  = get_query_var( 'bbppp_tab' );

$profile_id = ( $bbppp_user_obj && isset( $bbppp_user_obj->ID ) ) ? $bbppp_user_obj->ID : get_current_user_id();
...
if ( ! $user ) {
    ...
    status_header( 404 );
    get_header();
    echo '<p>' . esc_html__( 'User not found.', 'bbp-profile-plus' ) . '</p>';
    get_footer();
    exit;
}
...
<div id="bbppp-profile" class="bbppp-wrap" data-user="<?php echo esc_attr( $profile_id ); ?>">
    ...
    <div class="bbppp-profile-header">
        <div class="bbppp-avatar-wrap">
            <img src="<?php echo esc_url( $avatar_url ); ?>" alt="<?php echo esc_attr( $user->display_name ); ?>" class="bbppp-avatar">
            <?php if ( $is_own ) : ?>
                <a href="<?php echo esc_url( bbppp_get_account_url( 'general' ) ); ?>" class="bbppp-change-avatar"><?php esc_html_e( 'Change Photo', 'bbp-profile-plus' ); ?></a>
            <?php endif; ?>
        </div>
    ...
    $current_tab = $bbppp_tab_val ? $bbppp_tab_val : 'profile';
    ?>
                    ...
                    <div class="bbppp-profile-group" id="bbppp-group-<?php echo esc_attr( $group->id ); ?>">
                        ...
                        <table class="bbppp-profile-fields"><tbody>
                            <?php foreach ( $group->fields as $field ) : ?>
                                ...
                                if ( is_array( $value ) ) {
                                    $display = implode( ', ', array_map( 'esc_html', $value ) );
                                } else {
